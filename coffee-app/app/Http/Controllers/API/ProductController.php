...
class ProductController extends Controller
{
    ...
    {
        $query = Product::query()
            ->when(request('with'), function ($query, $with) {
            ...
            ->when(request('sort'), function ($query, $sort) {
                $order = request('order', 'asc'); // Mặc định là 'asc' nếu không truyền tham số
                if (in_array($sort, ['price'])) { // Chỉ cho phép sắp xếp theo cột 'price'
